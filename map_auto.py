...
def auto_map(path):
    if(os.path.exists('umc_mapcycle.txt')):
        os.remove('umc_mapcycle.txt')  # Check map file
    
    file_list = os.listdir(path)
    
    f = open('umc_mapcycle.txt', 'a', encoding='UTF-8', errors='ignore')
    print(r'''// Mapcycle definition for Ultimate Mapchooser v3.1 by Steell
...
"umc_mapcycle"
{
    "Gaybar" //Name this whatever you like
    {
        
        "maps_invote"   "20" ''', file = f)
    ...
    for i in range(len(file_list)):
        ...
        ''' % re.findall(r'(.+?).bsp',file_list[i])[0] , file = f)
